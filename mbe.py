...
#  'response':'81bc5c9d45fe548a4e7085bc5c7417f2799eb04fc409e8af8e0500800080'},
#  {'request':'0100000000f9babbbcbd',                                                   'response':'81781edc1e'},
#  {'request':'0100000000fa64656c',                                                     'response':'81482458'},
#  {'request':'0100000000fd202425264042434d',
#  'response':'81a900000180b00040'},
...
# Turns off calls to isotp layer and uses simple test data
test_mode = True

if test_mode:
    response_data = {
        '0xf8': b'\x81\xe7\x49\x21\x53\x00\x00\x3e\x97',
        '0xf9': b'\x81\x78\x1e\xdc\x1e'
    }
...
class mbe:
    ...
    def set_options(self, filename, txid=None, rxid=None, interface=None):
        """
    ...
    @staticmethod
    def load_mbe_variables_from_json(filename):
        """
        Load the JSON ec2 definitions from filename
        :param filename:
        :return:
        """
        try:
    ...
    def create_page_reverse_mapping(variables):
        ...
        for i in variables:
            ...
            if not page in mapping:
                mapping[page] = dict()
            mapping[page][LSBstring] = {
                'name': variables[i]['name'],
                'bytes': variables[i]['bytes']
            }
        return mapping
    ...
    # Add a variable to interrogate on the ECU (frequency of requests not currently supported)
    # We do this by creating a dict of pages, then put all the vars we want to follow as an ordered (lsb) list in
    # each pages that list will then be the vars that are constructed into a compound request to the ECU.   Not sure if
    # an ordered list is important but Easimap sends queries to the ECU in an ordered list so we'll do the same
    ...
    @staticmethod
    def create_data_request(page_name, page):
        """
    ...
    def process_data_response(self, response, vars_to_follow):
        ...
        for var in vars_to_follow:
            ...
            if i > data_length:
                logging.error(f"Ran off the end of our response array, counter got to {i} but array "
                              f"length is {data_length}")
                return False
